...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#FFFFFF' }}>
      <div className="text-center">
        ...
        <p style={{ color: '#059669', fontWeight: 800 }} className="animate-pulse">Cargando...</p>
      </div>
    </div>
  )
  ...
  const CAT_LABELS = {
    ...
    health:'Salud', entertainment:'Ocio', education:'Educación',
    savings:'Ahorro', salary:'Salario', freelance:'Freelance',
    other:'Otros', debt:'Deuda', clothing:'Ropa', utilities:'Servicios',
    subscriptions:'Suscripciones', credit_card:'Tarj. Crédito',
  }
    ...
    <div className="min-h-screen pb-28 page-transition" style={{ background: '#F9FAFB' }}>
        ...
        <div className="px-5 pt-3 pb-24">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 12 }}>Hola, {profile?.name?.split(' ')[0] || 'héroe'} 👋</p>
                <div className="flex items-center gap-2">
                  <span style={{ color: '#fff', fontWeight: 800, fontSize: 15 }}>Nivel {levelInfo.level}</span>
      ...
      <div className="px-4 -mt-16">
        <div style={{ background: '#fff', borderRadius: 24, padding: '20px 20px 16px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}>
          ...
          {/* Balance */}
          <p style={{ fontSize: 12, color: '#9CA3AF', fontWeight: 600, letterSpacing: 0.5, marginBottom: 4 }}>BALANCE DEL MES</p>
          ...
          {income > 0 && (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                <span style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 500 }}>Gastado del ingreso</span>
      ...
      <div className="px-4 mt-4 space-y-4">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <Link href="/transactions?type=income">
            <div style={{ background: '#059669', borderRadius: 18, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <p style={{ color: '#fff', fontWeight: 800, fontSize: 14 }}>Ingreso</p>
          ...
          <Link href="/transactions?type=expense">
            <div style={{ background: '#fff', borderRadius: 18, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10, border: '1.5px solid #FEE2E2' }}>
              ...
              <div>
                <p style={{ color: '#111827', fontWeight: 800, fontSize: 14 }}>Gasto</p>
        ...
        <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 16px 10px' }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>Movimientos</p>
            <Link href="/transactions">
              <span style={{ fontSize: 13, color: '#059669', fontWeight: 600 }}>Ver todos →</span>
            </Link>
          </div>
          {recentTxns.length === 0 ? (
            <div style={{ padding: '20px 16px 20px', textAlign: 'center' }}>
              <p style={{ fontSize: 28, marginBottom: 6 }}>📭</p>
              <p style={{ fontSize: 13, color: '#9CA3AF' }}>Sin movimientos este mes</p>
              <Link href="/transactions?type=expense">
                <p style={{ fontSize: 13, color: '#059669', fontWeight: 600, marginTop: 8 }}>+ Registrar primero</p>
              </Link>
            </div>
          ) : (
            <div>
              {recentTxns.map((t, i) => {
                ...
                const emoji = isIncome ? '💰' : { housing:'🏠', food:'🍔', transport:'🚗', health:'💊', entertainment:'🎮', education:'📚', savings:'🏦', other:'📦', debt:'⚔️', clothing:'👕', utilities:'💡', subscriptions:'📱', credit_card:'💳' }[t.category] || '📦'
                return (
                  <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderTop: i === 0 ? 'none' : '1px solid #F9FAFB' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {t.note || CAT_LABELS[t.category] || (isIncome ? 'Ingreso' : 'Gasto')}
                      </p>
                      <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: 1 }}>{t.date} · {CAT_LABELS[t.category] || t.category}</p>
                    </div>
        ...
        {activeMission ? (
          <Link href="/debt-dungeon">
            <div style={{ background: '#fff', borderRadius: 20, padding: '16px', border: '1.5px solid #FEF3C7', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 10 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#D97706', background: '#FEF3C7', padding: '2px 8px', borderRadius: 20 }}>⚔️ MISIÓN ACTIVA</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontWeight: 700, fontSize: 14, color: '#111827' }}>Derrota a {activeMission.boss_name || activeMission.name}</p>
                  <p style={{ color: '#9CA3AF', fontSize: 12, marginTop: 2 }}>{formatCurrency(activeMission.balance)} restante</p>
              ...
              {debts.length > 1 && (
                <p style={{ fontSize: 11, color: '#D97706', fontWeight: 600, marginTop: 10, textAlign: 'center' }}>
                  +{debts.length - 1} deuda{debts.length > 2 ? 's' : ''} más en el Dungeon
                </p>
              )}
            </div>
          </Link>
          ...
          <Link href="/debt-dungeon">
            <div style={{ background: '#fff', borderRadius: 20, padding: '16px', border: '1.5px dashed #E5E7EB', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              ...
              <p style={{ color: '#9CA3AF', fontSize: 12, marginTop: 3 }}>Agrega una deuda para batallar</p>
            </div>
          </Link>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
          {[
            { href: '/transactions', emoji: '📋', label: 'Moves' },
            { href: '/budget',       emoji: '📊', label: 'Budget' },
            { href: '/goals',        emoji: '🎯', label: 'Metas' },
            { href: '/reports',      emoji: '📈', label: 'Reports' },
          ].map(item => (
        ...
        <Link href="/coach">
          <div style={{ background: `${companion.color}0F`, borderRadius: 20, padding: '14px 16px', border: `1.5px solid ${companion.color}25` }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontWeight: 700, fontSize: 12, color: companion.color }}>💬 {companion.name} dice</p>
