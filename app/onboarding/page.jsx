...
export default function OnboardingPage() {
  ...
  function ProgressBar() {
    ...
    return (
      <div style={{ padding: '0 20px', paddingTop: 'max(20px, env(safe-area-inset-top))', background: '#FFFFFF' }}>
  ...
  if (currentStep === 'welcome') return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px 40px', background: '#FFFFFF', textAlign: 'center' }}>

      {/* Logo */}
      <div style={{ width: 90, height: 90, borderRadius: 28, background: 'linear-gradient(135deg, #059669, #10B981)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 44, marginBottom: 24, boxShadow: '0 12px 40px rgba(5,150,105,0.30)' }}>
        🎮
      </div>

      <h1 style={{ fontSize: 36, fontWeight: 900, color: '#111827', letterSpacing: -1, lineHeight: 1.1, margin: 0 }}>
        MyMoney <span style={{ color: '#059669' }}>GO</span>
      </h1>
      <p style={{ fontSize: 15, color: '#6B7280', marginTop: 10, lineHeight: 1.6, maxWidth: 280 }}>
        Convierte tus finanzas en una aventura épica. ¡Sube de nivel!
      </p>

      {/* Features */}
      <div style={{ width: '100%', maxWidth: 360, marginTop: 32, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {[
          { icon: '⚔️', title: 'Derrota tus deudas',        desc: 'Como jefes finales en el Debt Dungeon' },
          { icon: '📊', title: 'Controla tu presupuesto',    desc: 'Regla 50/30/20 personalizada para ti' },
          { icon: '⚡', title: 'Gana XP y sube de nivel',    desc: 'Cada decisión financiera te hace crecer' },
          { icon: '🦊', title: 'Tu compañero te guía',       desc: 'Un aliado en cada paso del camino' },
        ].map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 18, background: '#F9FAFB', border: '1.5px solid #F3F4F6', textAlign: 'left' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: '#9CA3AF', marginTop: 2 }}>{item.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <button onClick={() => setStep(1)}
        style={{ marginTop: 28, width: '100%', maxWidth: 360, padding: '17px', background: 'linear-gradient(135deg, #059669, #10B981)', color: '#FFFFFF', fontWeight: 900, fontSize: 16, borderRadius: 18, border: 'none', cursor: 'pointer', boxShadow: '0 8px 28px rgba(5,150,105,0.35)', letterSpacing: 0.3 }}>
        ¡Comenzar aventura! →
      </button>

      <p style={{ color: '#9CA3AF', fontSize: 12, marginTop: 14 }}>🎁 50 XP de bienvenida te esperan</p>

      {/* Login link */}
      <p style={{ fontSize: 13, color: '#6B7280', marginTop: 20 }}>
        ¿Ya tienes cuenta?{' '}
        <a href="/auth/login" style={{ color: '#059669', fontWeight: 700, textDecoration: 'none' }}>Inicia sesión</a>
      </p>
    </div>
  )
